...
- SvelteKit scaffold ปัจจุบัน build ได้แล้ว, root page อ่าน branding/counts จาก Drug Store Postgres แทน `countries`, login/protected layout ใช้ legacy session cookie แล้ว และ dashboard route อ่าน `getDashboard` จริง; receive/stock/exchange/settings routes ยังเป็น placeholder ยังไม่ใช่ UI จริงครบของ Drug Store
