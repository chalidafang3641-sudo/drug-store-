...
- SvelteKit `/receive` โหลด `getDrugs`, `getLocations`, `recentReceives`, ค้น/ยิง barcode ผ่าน `findDrugByCode`, export รับเข้าวันนี้ และ submit `receiveItem` ด้วย form action แล้ว
...
- SvelteKit `/settings` มี settings basic สำหรับ config, users, history filters, stock audit, master data ยา/สถานที่, upload รูปยา, export CSV และ notification config ผ่าน form action แล้ว
...
4. เติม camera scan ให้ SvelteKit receive ถ้าต้องการ parity เต็มกับ UI เดิม
5. เพิ่ม negative/permission tests สำหรับ write workflows
6. ทดสอบ export UI/settings save config เชิงลึก และตัดสินใจว่าจะต้องคง `.xlsx` แทน CSV หรือไม่
7. ทำ notification worker/cron ใหม่แทน Apps Script trigger และต่อ testNotification ให้ส่งจริง
8. Deploy preview บน Vercel พร้อมตั้ง env production
