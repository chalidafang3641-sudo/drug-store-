...
2. Scaffold SvelteKit structure/scripts ให้รันจริงได้
3. เปลี่ยน SvelteKit placeholder จาก `countries` เป็น domain table เช่น `locations`
4. Add `/api` compatibility endpoint ใน SvelteKit
5. Move current Node/Postgres API into SvelteKit server modules
6. Implement login + protected layout
7. Migrate dashboard page
8. เพิ่ม negative/permission tests สำหรับ write workflows
9. ทดสอบ export/history filters และ settings save config
10. ทำ notification worker/cron ใหม่แทน Apps Script trigger
11. Deploy preview บน Vercel พร้อมตั้ง env production
