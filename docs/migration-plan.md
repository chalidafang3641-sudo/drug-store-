...
- SvelteKit `/settings` มี settings basic สำหรับ config, users, history filters, stock audit และ master data ยา/สถานที่ผ่าน form action แล้ว
...
- [ ] Export/search ทำงานกับข้อมูล Supabaseครบทุก filter ใน SvelteKit UI
...
5. เติม upload รูปยาใน SvelteKit master data UI
6. เพิ่ม negative/permission tests สำหรับ write workflows
7. ทดสอบ export UI และ settings save config เชิงลึก
8. ทำ notification worker/cron ใหม่แทน Apps Script trigger
