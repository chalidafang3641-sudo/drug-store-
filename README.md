...
- SvelteKit scaffold build ได้แล้ว, login/protected layout ใช้ legacy `app_users` session ผ่าน httpOnly cookie แล้ว, dashboard/stock อ่านข้อมูลจริงจาก Postgres แล้ว, `/receive` รับเข้าผ่าน `receiveItem`, `/exchange` ย้ายยาผ่าน `exchangeItem`, `/stock` ตัดจ่ายผ่าน `disposeItem` และ `/settings` จัดการ config/users/history/audit/master data/export CSV ได้แล้ว; upload รูปยาใน UI และ notification ยังรอ migrate

ดังนั้นคำตอบเชิงสถานะคือ backend ใหม่รองรับ UI เดิมได้แล้วในงานหลักระดับ smoke test และ SvelteKit เริ่มมี auth/dashboard/stock/receive/exchange/stock dispose/settings/master data/export จริงแล้ว แต่ยังต้องทำ QA เชิงลึก, permission/edge cases, notification และ upload รูปยาใน UI ก่อน production cutover
