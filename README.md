...
- SvelteKit scaffold build ได้แล้ว, login/protected layout ใช้ legacy `app_users` session ผ่าน httpOnly cookie แล้ว, dashboard/stock อ่านข้อมูลจริงจาก Postgres แล้ว, `/receive` รับเข้าผ่าน `receiveItem` พร้อม barcode/search, HID/camera scan และ export วันนี้, `/exchange` ย้ายยาผ่าน `exchangeItem`, `/stock` ตัดจ่ายผ่าน `disposeItem` และ `/settings` จัดการ config/users/history/audit/master data/upload รูปยา/export CSV/notification config ได้แล้ว
- หน้าที่ยังไม่ parity กับ UI เดิมชัดที่สุดตอนนี้คือ `/login`; flow auth ใช้ได้แล้วแต่ layout/branding/visual hierarchy ยังไม่ตรง legacy และต้องเก็บให้เหมือนเดิมก่อนค่อย polish ส่วนอื่น
- notification worker/cron ยังรอทำ

ดังนั้นคำตอบเชิงสถานะคือ backend ใหม่รองรับ UI เดิมได้แล้วในงานหลักระดับ smoke test และ SvelteKit เริ่มมี auth/dashboard/stock/receive/exchange/stock dispose/settings/master data/upload/export/notification config จริงแล้ว แต่ยังต้องปิด gap ฝั่ง UI parity โดยเฉพาะหน้า login พร้อม QA เชิงลึก, permission/edge cases และ notification worker/cron ก่อน production cutover
