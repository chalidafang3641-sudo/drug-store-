...
- SvelteKit scaffold build ได้แล้ว, login/protected layout ใช้ legacy `app_users` session ผ่าน httpOnly cookie แล้ว, dashboard route อ่าน `getDashboard` จริงจาก Postgres แล้ว, และ `/stock` อ่าน stock ตามสถานที่จริงแล้ว; routes งานเขียน/ตั้งค่ายังเป็น placeholder รอ migrate UI จริง

ดังนั้นคำตอบเชิงสถานะคือ backend ใหม่รองรับ UI เดิมได้แล้วในงานหลักระดับ smoke test และ SvelteKit เริ่มมี auth/dashboard/stock read-only จริงแล้ว แต่ยังต้องทำ QA เชิงลึก, permission/edge cases, notification/export และย้าย UI งานเขียนที่เหลือเข้า SvelteKit ก่อน production cutover
