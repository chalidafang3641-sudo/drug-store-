...
- SvelteKit scaffold build ได้แล้ว, login/protected layout ใช้ legacy `app_users` session ผ่าน httpOnly cookie แล้ว, dashboard/stock อ่านข้อมูลจริงจาก Postgres แล้ว, และ `/receive` รับเข้าด้วย form action ผ่าน `receiveItem` แล้ว; routes แลกยา/ตั้งค่ายังเป็น placeholder รอ migrate UI จริง

ดังนั้นคำตอบเชิงสถานะคือ backend ใหม่รองรับ UI เดิมได้แล้วในงานหลักระดับ smoke test และ SvelteKit เริ่มมี auth/dashboard/stock/receive จริงแล้ว แต่ยังต้องทำ QA เชิงลึก, permission/edge cases, notification/export และย้าย UI งานเขียนที่เหลือเข้า SvelteKit ก่อน production cutover
