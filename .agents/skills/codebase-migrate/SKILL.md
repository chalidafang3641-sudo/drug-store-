...
description: Run Drug Store large codebase migrations and multi-file refactors in local reviewable batches. Use for static-to-SvelteKit migration, schema cleanup, repeated transforms, auth/permission refactors, and module-by-module modernization.
metadata:
...
Follow `AGENTS.md` first. For Drug Store, good batches are page/workflow slices, master data groups, auth/permission flows, static-to-SvelteKit route groups, DB migration groups, or repeated code transforms.
