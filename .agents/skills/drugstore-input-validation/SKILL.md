---
name: drugstore-input-validation
description: Project-specific validation checklist for Drug Store forms, API payloads, stock records, master data, and database-backed write flows.
---

# Drug Store Input Validation
...
- Drug codes/barcodes: preserve exact string value, trim surrounding whitespace, reject control characters, and enforce uniqueness for active drugs.
- Lot numbers: preserve exact visible value except surrounding whitespace; require them only when the drug master marks `require_lot`.
...
- Stock quantities: require non-negative integers for counted stock and positive integers for receive/move/dispose actions.
- Dates: validate format, valid calendar dates, and start/end ordering.
- Expiry dates: require valid date-only values and keep date handling timezone-safe.
